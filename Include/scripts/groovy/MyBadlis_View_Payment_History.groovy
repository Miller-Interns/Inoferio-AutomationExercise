...
class MyBadlisViewPaymentHistorySteps {
	...
	@Before("@MyBadlisViewPaymentHistory")
	def beforeScenario() {
		loadEnvVariables()
		WebUI.openBrowser('')
	...
	@After("@MyBadlisViewPaymentHistory")
	def afterScenario() {
		WebUI.closeBrowser()
	}
	...
	def loginAndOpenDashboardForPaymentHistory() {
	...
	def openMyBadlisAndSelectPaymentHistoryTab() {
	...
	def verifyPaymentHistoryTabDisplayed() {
		assert driver.getCurrentUrl().contains('/badlis') : 'Failed to navigate to My Badlis page'
		WebUI.delay(3)
		KeywordUtil.logInfo('Payment History tab displayed')
	}
}
