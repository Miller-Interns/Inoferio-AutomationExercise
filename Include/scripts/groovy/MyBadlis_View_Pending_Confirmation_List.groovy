...
class MyBadlisViewPendingConfirmationListSteps {
	...
	@Before("@MyBadlisViewPendingList")
	def beforeScenario() {
		loadEnvVariables()
		WebUI.openBrowser('')
	...
	def loginAndOpenDashboardForPendingList() {
	...
	def openMyBadlisAndSelectPendingConfirmationTab() {
		...
		try {
			driver.findElement(MY_BADLIS_MENU).click()
		} catch (Exception e) {
	...
	def verifyPendingConfirmationTabDisplayed() {
		assert driver.getCurrentUrl().contains('/badlis') : 'Failed to navigate to My Badlis page'
		WebUI.delay(3)
		KeywordUtil.logInfo('Pending Confirmation tab displayed')
	}
}
