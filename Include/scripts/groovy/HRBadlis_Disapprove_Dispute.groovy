...
class HRBadlisDisapproveDisputeSteps {
	...
	@Before("@HRBadlisDisapproveDispute")
	def beforeScenario() {
		loadEnvVariables()
		WebUI.openBrowser('')
	...
	@After("@HRBadlisDisapproveDispute")
	def afterScenario() {
		WebUI.closeBrowser()
	}
	...
	def loginAndOpenDashboardForDisapproveDispute() {
	...
	def openHRBadlisRecordsAndDisputesTabForDisapprove() {
	...
	@When("the user expands a dispute row and clicks Reject")
	def expandDisputeRowAndClickReject() {
		WebUI.delay(2)
		...
		By rejectBtn = By.xpath("//button[@id='reject-button'] | //button[normalize-space()='Reject'] | //*[contains(@class,'p-button')][.//span[normalize-space()='Reject']] | //span[normalize-space()='Reject']/parent::*")
		wait.until(ExpectedConditions.presenceOfElementLocated(rejectBtn))
		def btn = driver.findElement(rejectBtn)
		js.executeScript("arguments[0].scrollIntoView({block:'center'});", btn)
		WebUI.delay(0.3)
		try { wait.until(ExpectedConditions.elementToBeClickable(rejectBtn)).click() } catch (Exception e) { js.executeScript("arguments[0].click();", btn) }
		WebUI.delay(0.5)
	}

	@When("the user enters rejection reason \"(.*)\" and submits")
	def enterRejectionReasonAndSubmit(String reason) {
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//*[contains(@class,'p-dialog')] | //*[@role='dialog']")))
		By reasonTextarea = By.xpath("//textarea[@id='reason-for-rejection'] | //*[contains(@class,'p-dialog') or @role='dialog']//textarea[@id='reason-for-rejection'] | //textarea[@id='reason-for-rejection']")
		wait.until(ExpectedConditions.presenceOfElementLocated(reasonTextarea))
		def ta = driver.findElement(reasonTextarea)
		...
		try {
			...
			js.executeScript("arguments[0].value = arguments[1];", ta, reason)
			js.executeScript("arguments[0].dispatchEvent(new Event('input', { bubbles: true }));", ta)
		}
		WebUI.delay(0.5)
		By submitBtn = By.xpath("//*[contains(@class,'p-dialog') or @role='dialog']//button[normalize-space()='Submit'] | //button[normalize-space()='Submit']")
		wait.until(ExpectedConditions.presenceOfElementLocated(submitBtn))
		def btn = driver.findElement(submitBtn)
		js.executeScript("arguments[0].scrollIntoView({block:'center'});", btn)
		WebUI.delay(0.3)
		try { wait.until(ExpectedConditions.elementToBeClickable(submitBtn)).click() } catch (Exception e) { js.executeScript("arguments[0].click();", btn) }
		WebUI.delay(1)
	}

	@Then("the dispute is disapproved")
	def verifyDisputeDisapproved() {
		WebUI.delay(3)
		KeywordUtil.logInfo('Dispute disapproved')
	}
}
