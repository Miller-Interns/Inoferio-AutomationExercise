...
class MyBadlisPaymentHistoryFilterSteps {
	...
	@Before("@MyBadlisPaymentHistoryFilter")
	def beforeScenario() {
		loadEnvVariables()
		WebUI.openBrowser('')
	...
	@After("@MyBadlisPaymentHistoryFilter")
	def afterScenario() {
		WebUI.closeBrowser()
	}
	...
	def loginAndOpenDashboardForPaymentHistoryFilter() {
	...
	def openMyBadlisAndPaymentHistoryForFilter() {
	...
	@When("the user applies a date filter on Payment History")
	def applyDateFilterOnPaymentHistory() {
		WebUI.delay(1)
		By dateCell = By.xpath("//span[normalize-space()='28']")
		if (driver.findElements(dateCell).size() > 0) {
			wait.until(ExpectedConditions.elementToBeClickable(dateCell)).click()
			WebUI.delay(1)
		}
	}

	@When("the user clicks Clear Filters on Payment History")
	def clickClearFiltersOnPaymentHistory() {
	...
	def verifyPaymentHistoryFiltersCleared() {
		WebUI.delay(3)
		KeywordUtil.logInfo('Payment History filters cleared')
	}
}
