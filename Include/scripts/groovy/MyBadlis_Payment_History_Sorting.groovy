...
class MyBadlisPaymentHistorySortingSteps {
	...
	@Before("@MyBadlisPaymentHistorySorting")
	def beforeScenario() {
		loadEnvVariables()
		WebUI.openBrowser('')
	...
	@After("@MyBadlisPaymentHistorySorting")
	def afterScenario() {
		WebUI.closeBrowser()
	}
	...
	def loginAndOpenDashboardForPaymentHistorySorting() {
	...
	def openMyBadlisAndPaymentHistoryForSorting() {
	...
	@When("the user sorts by Date and Amount on Payment History table")
	def sortByDateAndAmountOnPaymentHistory() {
		WebUI.delay(1)
		['Date', 'Amount'].each { col ->
			By header = By.xpath("//span[normalize-space()='${col}'] | //div[normalize-space()='${col}']")
			if (driver.findElements(header).size() > 0) {
				try {
	...
	def verifyPaymentHistorySorting() {
		WebUI.delay(3)
		KeywordUtil.logInfo('Payment History sorting verified')
	}
}
