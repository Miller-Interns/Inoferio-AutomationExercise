...
class HRBadlisViewBadlisRecordsSteps {
	...
	@Before("@HRBadlisViewBadlisRecords")
	def beforeScenario() {
		loadEnvVariables()
		WebUI.openBrowser('')
	...
	@After("@HRBadlisViewBadlisRecords")
	def afterScenario() {
		WebUI.closeBrowser()
	}
	...
	def loginAndOpenDashboardForHRBadlisRecords() {
	...
	def openHRBadlisRecordsPageAndSelectBadlisRecordsTab() {
	...
	def verifyHRBadlisRecordsTabDisplayed() {
		assert driver.getCurrentUrl().contains('/admin') : 'Failed to navigate to HR Badlis Records'
		WebUI.delay(3)
		KeywordUtil.logInfo('HR Badlis Records tab displayed')
	}
}
